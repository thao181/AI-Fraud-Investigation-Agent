...
A transaction enters n8n through an API webhook.

A FastAPI ML service calculates a fraud-risk score from 0 to 100.

Transactions scoring below 60 follow the standard monitoring path.

Transactions scoring 60 or above are escalated to the AI investigation agent.

The agent must search the fraud-policy knowledge base before making a
policy-based recommendation.

The result is returned as structured JSON, logged in Google Sheets, and sent
to the fraud team by email.

Every escalated case is marked HUMAN_REVIEW_REQUIRED.
...
Detects risk patterns and produces a consistent transaction score.

RAG

Retrieves relevant fraud-policy text instead of relying on the language model's memory.

AI agent

Combines transaction evidence, model signals, and retrieved policy into an investigation summary and recommendation.

n8n

Controls routing, integrations, audit logging, alerts, and the final API response.

Human analyst

Reviews the evidence and makes the final customer-impacting decision.
...
OpenAI embeddings, n8n in-memory vector store

Integrations

Gmail, Google Sheets, webhooks
...
Log and continue standard monitoring.
...
Escalate to AI investigation.

70–100

High

Escalate, alert the fraud team, and require human review.
...
Render reads render.yaml and builds ml_api/Dockerfile.

Confirm the /health endpoint returns a healthy status.

Copy the Render service URL and add /predict.

Update the n8n node Score Transaction Risk (ML API) with that URL.
...
Add credentials for OpenAI, Gmail, and Google Sheets.

In Log Case in Google Sheets, select your spreadsheet and use the column
structure in docs/google_sheets_columns.csv.

In Email High-Risk Alert, replace
YOUR_FRAUD_TEAM_EMAIL@example.com with the destination address.

Confirm the ML scoring node uses your Render /predict endpoint.

Run Run Knowledge Base Setup once to index the demonstration fraud
policies.

Activate the workflow and test the production webhook:
